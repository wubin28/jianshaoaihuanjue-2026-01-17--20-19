...
```
我是spec-kit的新手。请你阅读当前项目的 @.specify/ 和 @.claude/ 目录下的脚本和markdown文件，然后解释下面的speckit命令是如何通过这些脚本和markdown起作用的，以便当这些命令的行为需要调整时，我能知道如何操作。下面是我关注的5个命令：【
...
/speckit.tasks
/speckit.implement
】。我想通过理解底层脚本和模板的工作机制，来掌握如何调整这些命令的行为以满足我的项目需求。输出格式为markdown，并为每个主要观点提供3个不同出处的网页链接。我担心你给出的解释可能不准确，或者提供的链接无法有效验证信息。
请严格按照以下工作流程完成我的诉求：
【## 背景说明
